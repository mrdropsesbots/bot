    ...
    try:
        ...
        cur.execute(
            ...
            (
                ...
                float(data['latitude']), float(data['longitude']), desc
            )
        )
        conn.commit()
        conn.close()
        # Готовая строка для Google Таблицы
        row_csv = f"{data['name']}, {data['category']}, {data['address']}, {data['latitude']}, {data['longitude']}, {desc}"
        await msg.answer(
...
async def do_clear(msg: Message, state: FSMContext):
    if msg.text == "ДА":
        ...
        conn.commit()
        conn.close()
        await msg.answer("✅ База данных полностью очищена.", reply_markup=admin_menu())
    else:
        await msg.answer("❌ Очистка отменена.", reply_markup=admin_menu())
    await state.clear()
...
async def process_venues_csv(msg: Message, reader):
    ...
    for row_num, row in enumerate(reader, start=2):
        try:
            ...
            if not cat:
                ...
                cat_id = cat[0]
            cur.execute(
                "INSERT INTO venues (name, category_id, address, latitude, longitude, description) VALUES (?,?,?,?,?,?)",
    ...
    if dups:
        ...
        reply += f"\n⚠️ Обнаружены дубликаты названий: {dup_names}. Рекомендуется сделать их уникальными."
    if errors:
