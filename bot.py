...
async def add_restaurant_cmd(message: Message):
    ...
    try:
        ...
        c = await conn.execute(
            "INSERT INTO restaurants (name, cuisine, address, lat, lon, phone, work_hours, avg_check) VALUES (?,?,?,?,?,?,?,?)",
...
async def add_dish_cmd(message: Message):
    ...
    try:
        ...
        rid = rows[0][0]
        await conn.execute(
            "INSERT INTO menu (restaurant_id, category, dish, description, price) VALUES (?,?,?,?,?)",
...
async def edit_cmd(message: Message):
    ...
    try:
        ...
        allowed = {
            "restaurant": {"name", "cuisine", "address", "phone", "work_hours", "avg_check"},
    ...
    try:
        ...
        for mid, price in rows:
            if action.startswith("+"):
                ...
                raise ValueError
            await conn.execute("UPDATE menu SET price = ? WHERE id = ?", (new_price, mid))
            updated += 1
        await conn.commit()
        await conn.close()
        await message.answer(f"✅ Обновлено {updated} блюд.\nФильтр: {rname} / {category}\nДействие: {action}")
    except Exception as e:
...
async def export_csv(message: Message):
    if message.from_user.id != ADMIN_ID:
        return
    conn = await get_conn()
    rests = await conn.execute_fetchall("SELECT id, name, cuisine, address, lat, lon, phone, work_hours, avg_check FROM restaurants")
    dishes = await conn.execute_fetchall("SELECT m.id, r.name, m.category, m.dish, m.description, m.price FROM menu m JOIN restaurants r ON m.restaurant_id = r.id")
    await conn.close()
    out1 = io.StringIO()
    w1 = csv.writer(out1)
    w1.writerow(["id", "name", "cuisine", "address", "lat", "lon", "phone", "work_hours", "avg_check"])
    for r in rests:
        w1.writerow(r)
    out1.seek(0)
    out2 = io.StringIO()
    w2 = csv.writer(out2)
    w2.writerow(["id", "restaurant", "category", "dish", "description", "price"])
    for d in dishes:
        w2.writerow(d)
    out2.seek(0)
    await message.answer_document(BufferedInputFile(out1.getvalue().encode("utf-8"), "restaurants.csv"))
    await message.answer_document(BufferedInputFile(out2.getvalue().encode("utf-8"), "menu.csv"))
    ...
    try:
        ...
        if "restaurant" in headers or "name" in headers:
            for row in reader:
                ...
                await conn.execute(
                    "INSERT INTO restaurants (name, cuisine, address, lat, lon, phone, work_hours, avg_check) VALUES (?,?,?,?,?,?,?,?)",
                    (name, row.get("cuisine", ""), row.get("address", ""), float(row.get("lat", 0)), float(row.get("lon", 0)),
                     row.get("phone", ""), row.get("work_hours", ""), float(row.get("avg_check", 0) or 0))
                )
                added += 1
            ...
            for row in reader:
                ...
                await conn.execute(
                    "INSERT INTO menu (restaurant_id, category, dish, description, price) VALUES (?,?,?,?,?)",
                    (rid, row.get("category", "Разное"), row["dish"], row.get("description", ""), float(row["price"]))
                )
                added += 1
        await conn.commit()
        await conn.close()
...
async def stats_cmd(message: Message):
    ...
    text = (
        ...
        f"🔍 Поисков: {searches or 0} | 📍 Геолокаций: {locs or 0}\n"
