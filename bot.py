...
# --------------------- Состояния FSM (только для админки) ---------------------
...
# Добавление заведения (без изменений, но с поддержкой phone)
...
async def add_item_cat(msg: Message, state: FSMContext):
    ...
    try:
        ...
        if cnt > 1:
            ...
            await msg.answer(f"❌ Несколько заведений с таким названием:\n{addr_list}\nУточните название.")
            conn.close()
            await state.clear()
            return
...
async def process_venues_csv(msg: Message, reader):
    ...
    for row_num, row in enumerate(reader, start=2):
        try:
            ...
            phone = row.get('phone', '')  # безопасно, даже если колонки нет
    ...
    for row_num, row in enumerate(reader, start=2):
        try:
            ...
            if cnt == 0:
                ...
                continue
            if cnt > 1:
                skipped += 1  # дубликаты пропускаем
                continue
            cur.execute("SELECT id FROM venues WHERE name=?", (venue_name,))
            venue_id = cur.fetchone()[0]
            cur.execute("INSERT INTO menu_items (venue_id, name, price, category) VALUES (?,?,?,?)",
