...
async def init_db():
    ...
    # Check if old schema exists (missing lat/lon columns)
    needs_reset = False
    try:
        await conn.execute("SELECT lat FROM restaurants LIMIT 1")
    except Exception:
        needs_reset = True
    ...
    if needs_reset:
        ...
        await conn.execute("DROP TABLE IF EXISTS menu")
        await conn.execute("DROP TABLE IF EXISTS restaurants")
        await conn.execute("DROP TABLE IF EXISTS users")
        logging.info("Old database schema detected. Recreated tables.")
